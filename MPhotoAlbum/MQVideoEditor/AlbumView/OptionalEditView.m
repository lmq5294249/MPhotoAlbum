...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LocalAssetModel *model = [self.optionalDict objectForKey:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
    EditUnitModel *editUnitModel;
    if (self.templateModelArray.count > indexPath.row) {
        editUnitModel = self.templateModelArray[indexPath.row];
    }
    [cell displayCellWithModel:model templateModel:editUnitModel];
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _flowLayout.minimumLineSpacing = 16.0;//设置最小行间距为0
        _flowLayout.minimumInteritemSpacing = 16.0;
        _flowLayout.itemSize = CGSizeMake(self.frame.size.height, self.frame.size.height);
...
- (NSInteger)getNextIndex
{
    ...
    _curIndex = index;
    self.currentCount = selectedCount;
    if (_curIndex == 10000) {
        self.completed = YES;
    }
    else{
