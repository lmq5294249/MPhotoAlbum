...
- (void)setUpUI
{
    ...
    //拖动的替身显示
    _dragingItem = [[MQDragingCell alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.height, self.frame.size.height)];
...
//    videoEditView.transitionNodeArray = self.transitionNodeArray;
    [self.delegate startToEditSingleVideoWithIndex:indexPath.row];
}
...
-(void)rearrangeMediaFileArray
{
    ...
    [self.clipTimeRanges exchangeObjectAtIndex:drageIndex withObjectAtIndex:targetIndex];
    isChangeOrder = YES;
 }
